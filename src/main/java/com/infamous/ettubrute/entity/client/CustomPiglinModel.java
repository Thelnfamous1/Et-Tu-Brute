...
public class CustomPiglinModel<T extends MobEntity> extends PlayerModel<T> {
    ...
    public CustomPiglinModel(float v, int widthIn, int heightIn) {
        ...
        this.piglinBody = this.bipedBody.getModelAngleCopy(); // copies whatever is done to the biped version
        this.piglinHead = this.bipedHead.getModelAngleCopy(); // copies whatever is done to the biped version
        this.piglinLeftArm = this.bipedLeftArm.getModelAngleCopy(); // copies whatever is done to the biped version
        this.piglinRightArm = this.bipedLeftArm.getModelAngleCopy(); // copies whatever is done to the biped version
    }
    ...
    public void setRotationAngles(T p_225597_1_, float p_225597_2_, float p_225597_3_, float p_225597_4_, float p_225597_5_, float p_225597_6_) {
        ...
        if (p_225597_1_ instanceof AbstractPiglinEntity) {
            AbstractPiglinEntity lvt_10_1_ = (AbstractPiglinEntity)p_225597_1_;
            PiglinAction lvt_11_1_ = lvt_10_1_.func_234424_eM_();
            if (lvt_11_1_ == PiglinAction.DANCING) {
        ...
        } else if (p_225597_1_.getType() == EntityType.ZOMBIFIED_PIGLIN || p_225597_1_.getType() == ModEntityTypes.ZIGLIN_BRUTE.get()) {
